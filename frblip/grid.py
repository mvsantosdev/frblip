...
class CartesianGrid(object):
    def __init__(self, grid, xrange, yrange, alt=90, az=0.0):
        ...
        if grid.ndim == 3:
            ...
            self.patterns = [
                RectBivariateSpline(y, x, g, kx=3, ky=3) for g in grid
            ]
    ...
    def __unique_grid(self, altaz):
        ...
        altazoffs = [
            altaz.transform_to(self.offsets[i]) for i in range(self.n_beam)
        ]
        ...
        y = numpy.column_stack(
            [altazoff.cartesian.y for altazoff in altazoffs]
        )
    ...
    def __multiple_grids(self, altaz):
        ...
        return numpy.column_stack(
            [pattern.ev(y, x) for pattern in self.patterns]
        )
